...
-- Great Merchant
UPDATE Language_ko_KR
SET Text = '[ICON_GREAT_MERCHANT] 위대한 상인은 특별한 [COLOR_POSITIVE_TEXT]소도시[ENDCOLOR] 시설을 건설할 수 있습니다. 소도시는 인접 보너스를 활성화하고 많은 [ICON_GOLD] 골드와 [ICON_FOOD] 식량을 제공합니다. 또한 위대한 상인은 도시 국가로 이동하여 [COLOR_POSITIVE_TEXT]무역 임무[ENDCOLOR]를 할 수 있습니다. 무역 임무는 대량의 [ICON_GOLD] 골드를 벌어들이고 즉시 모든 도시가 국왕 경축일을 맞이하게 합니다. 무역 임무는 소유한 소도시마다 효과가 20% 증가합니다. [NEWLINE][NEWLINE]위 능력 중 하나를 사용하면 위대한 상인이 사라집니다.[NEWLINE][NEWLINE]소도시는 두 도시 사이를 연결하는 도로 위에 있다면 [ICON_GOLD] 골드와 [ICON_PRODUCTION] 생산을 +1 제공합니다. 철도라면 [ICON_GOLD] 골드와 [ICON_PRODUCTION] 생산을 +2 제공합니다. 소도시를 지나가는 교역로가 있다면 추가 [ICON_GOLD] 골드와 [ICON_PRODUCTION] 생산을 제공합니다(도로에서 +1, 철도에서 +2).'
...
-- Explorer
UPDATE Language_ko_KR
SET Text = '탐험가는 중세와 르네상스 시대에 바다를 탐험할 때 캐러벨과 함께 유용한 역할을 했습니다. 그들은 상당히 약하기 때문에 현대 군대에 의해 쉽게 패배할 수 있습니다. 그러나 그들의 정찰 능력은 코만도가 등장하기 전까지는 비교할 수 없을 정도입니다.'
...
-- Destroyer
UPDATE Language_ko_KR
SET Text = '구축함은 승급하면 근처의 잠수함을 드러낼 수 있는 해상 근접 유닛입니다. 다른 유닛을 공격하고 호위하는 데 사용합니다.'
...
UPDATE Language_ko_KR
SET Text = '산병은 빠른 원거리 유닛으로, 개방된 지역에서 강합니다. 이전의 궁전차와 다르게, 이동력 패널티 없이 험지에 들어갈 수 있으며 측면 공격에 강합니다. 기병 유닛이므로 산병은 창병에 취약합니다.'
...
-- Mehal Sefari
UPDATE Language_ko_KR
SET Text = '{TXT_KEY_UNIT_MEHAL_SEFARI}는 아군 영토에서 싸울 때, 특히 [ICON_CAPITAL] 수도 근처에서 싸울 때 상당한 전투 보너스를 얻습니다.'
