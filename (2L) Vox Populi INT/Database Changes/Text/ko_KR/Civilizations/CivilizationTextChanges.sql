...
UPDATE Language_ko_KR
SET Text = '{TXT_KEY_UNIT_ARCHER}을 대체하는 바빌론의 고유 유닛입니다. 일반 궁병보다 더 강력하고 근접 공격을 더 잘 견뎌냅니다. 부상 당한 유닛을 공격하면 측면 공격력이 증가합니다.'
...
UPDATE Language_ko_KR
SET Text = '{TXT_KEY_DESC_CUIRASSIER}을 대체하는 인도의 고유 유닛입니다. [ICON_RES_HORSE] {TXT_KEY_RESOURCE_HORSES}을 요구하지 않습니다. [ICON_MOVES] 이동력 1과 [COLOR_NEGATIVE_TEXT]{TXT_KEY_PROMOTION_SKIRMISHER_DOCTRINE}[ENDCOLOR] 승급을 잃는 대신 [ICON_STRENGTH] 전투력과 [ICON_RANGE_STRENGTH] 원거리 전투력이 강하며, 주변 적 유닛의 [ICON_STRENGTH] 전투력을 감소시킵니다. [COLOR_POSITIVE_TEXT]{TXT_KEY_PROMOTION_ACCURACY_1}[ENDCOLOR] 승급을 보유합니다.'
...
UPDATE Language_ko_KR
SET Text = '{TXT_KEY_UNIT_GREAT_GENERAL}을 대체하는 몽골의 고유 위인입니다. {TXT_KEY_UNIT_MONGOL_KHAN}은 같은 [ICON_STRENGTH][COLOR_POSITIVE_TEXT]{TXT_KEY_PROMOTION_GREAT_GENERAL} 보너스[ENDCOLOR]를 제공할 뿐만 아니라 [ICON_MOVES]이동력을 [COLOR_POSITIVE_TEXT]5[ENDCOLOR] 가지고 있으며 같은 타일 및 인접 타일의 아군 유닛이 체력 회복 시 턴마다 [COLOR_POSITIVE_TEXT]추가 회복[ENDCOLOR]을 할 수 있게 해줍니다. [NEWLINE][NEWLINE]몽골의 고유 [COLOR_POSITIVE_TEXT]{TXT_KEY_IMPROVEMENT_CITADEL}[ENDCOLOR]인 {TXT_KEY_IMPROVEMENT_MONGOLIA_ORDO}를 건설할 수 있습니다.'
...
UPDATE Language_ko_KR
SET Text = '도시가 공격으로부터 받는 피해를 2 감소시킵니다. 도시가 시야를 무시하고 공격할 수 있게 됩니다. [ICON_SILVER_FIST] 군사 유닛 보급 한도가 [ICON_CITIZEN]시민의 [COLOR_POSITIVE_TEXT]10%[ENDCOLOR]만큼 증가합니다.[NEWLINE][NEWLINE]적 지상 유닛이 이 도시의 지상 타일에 진입할 때 [ICON_MOVES]이동력을 모두 소모합니다.[NEWLINE][NEWLINE]야영지, 광산, 제재소, 전략 자원의 [ICON_PRODUCTION]생산 및 [ICON_GOLD] 골드가 [COLOR_POSITIVE_TEXT]1[ENDCOLOR] 증가합니다.[NEWLINE][NEWLINE]도시에 주둔한 유닛이 회복할 때 체력을 추가로 5만큼 더 회복합니다.[NEWLINE][NEWLINE] [ICON_CITY_STATE] 도시 규모 요구치가 [COLOR_POSITIVE_TEXT]5%[ENDCOLOR] 감소합니다.'
