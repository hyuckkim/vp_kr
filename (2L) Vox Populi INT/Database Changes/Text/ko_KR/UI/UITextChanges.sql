...
UPDATE Language_ko_KR
SET Text = '[ICON_CITY_STATE] 도시 국가에서는 기술을 훔칠 수 없으므로 [ICON_CITY_SECURITY] 보안 등급도 없습니다.[NEWLINE][NEWLINE][ICON_CITY_STATE] 도시 국가에 [ICON_SPY] 스파이를 보내면 [ICON_PUPPET] 선거 결과를 조작해 [ICON_INFLUENCE] 영향력을 상승시킬 수 있습니다. 수 있습니다. 한 도시 국가에 스파이가 여러 명 있다면 가장 레벨이 높은 스파이 중 해당 도시에 가장 오래 잠입했던 스파이가 선거 결과를 조작할 확률이 가장 높습니다.'
...
UPDATE Language_ko_KR
SET Text = '{1_RankName} {2_SpyName}{2: plural 1?은; 2?는;} {3_CityName}에서 우리의 [ICON_INFLUENCE] 영향력을 늘리기 위해 [ICON_PUPPET] 선거 조작을 시도하고 있습니다. [NEWLINE][NEWLINE]한 문명만이 선거 결과를 조작할 수 있습니다.한 도시 국가에 스파이가 여러 명 있다면 가장 계급이 높은 스파이 중 가장 그곳에 오래 머문 스파이가 선거 결과를 조작할 확률이 가장 높습니다. 연속으로 선거 조작에 성공하면 더 많은 영향력을 얻을 수 있습니다.[NEWLINE][NEWLINE][COLOR_POSITIVE_TEXT]선거 조작에 성공하면  {4_Influence} 영향력을 얻습니다.[ENDCOLOR]'
...
UPDATE Language_ko_KR
SET Text = '{1_SpyRank} {2_SpyName}의 스파이 옵션:[NEWLINE][NEWLINE][NEWLINE][NEWLINE][ICON_BULLET] 도시 국가로 이동해서 선거조작 가능[NEWLINE][ICON_BULLET] 주요 문명의 도시로 이동해서 기밀을 얻고 스파이 활동[NEWLINE][ICON_BULLET] 주요 문명의 수도로 이동해서 [ICON_DIPLOMAT]외교관 활동[NEWLINE][ICON_BULLET] 소유한 도시로 이동해서 [COLOR_POSITIVE_TEXT]역스파이 활동[ENDCOLOR].'
...
UPDATE Language_ko_KR
SET Text = '[NEWLINE][NEWLINE][COLOR_POSITIVE_TEXT]이 도시 국가는 강력한 문화적 영향을 받고 있는 문명과 동맹 관계이기 때문에, 선거 조작 중 스파이의 등급이 {1_SpyBonus}단계 상승합니다. [ENDCOLOR]'
...
UPDATE Language_ko_KR
SET Text = '[COLOR_POSITIVE_TEXT]실천적[ENDCOLOR]인 도시 국가는 당신에게 자신들을 보호해 줄 것을 약속해 달라고 요청하거나, 주변의 다른 도시 국가들을 괴롭히거나, 첩보 활동을 벌일 가능성이 더 높습니다.'
...
UPDATE Language_ko_KR
SET Text = '아직 두 플레이어 모두 이 항목을 거래할 수 있는 기술(교육)을 보유하고 있지 않습니다.'
